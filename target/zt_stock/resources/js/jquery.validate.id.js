...
$.extend($.validator, {
	...
	defaults: {
		...
		onkeyup: function(element, event) {
			if ( event.which === 9 && this.elementValue(element) === '' ) {
				return;
			} else if ( element.id in this.submitted || element === this.lastActive ) {
				this.element(element);
			}
		},
		...
		init: function() {
			...
			$(this.currentForm)
				.validateDelegate(":text, [type='password'], [type='file'], select, textarea, " +
					...
					"[type='email'],[type='isPhone'], [type='datetime'], [type='date'], [type='month'], " +
	...
	methods: {
		...
		isPhone:function(value, element){
		     ...
		     var mobile = /^0?(13[0-9]|15[012356789]|17[0-9]|18[0-9]|14[57])[0-9]{8}$/;
