...
{
	alert("att_show"+serverData+","+file);
	var serverData = serverData.replace(/<div.*?<\/div>/g,'');
	$("#return_data").val(serverData);
	...
	var img = "";
	if(ext == 1) {
	    img = '<div class="get selected" id="aid-'+id+'"><a class="del" href="javascript:;">删除</a> <img onclick="att_cancel(this,'+id+',\'upload\')" width="87" height="98" src="'+src+'" data-id="'+id+'" data-path="'+src+'" alt="上传完成" title="'+filename+'"><input type="hidden" id="fileId" value="'+id+'"/><input type="text" class="J_file_desc" id="fileName" name="flashatt['+id+'][desc]" placeholder="请输入描述" value="'+filename+'" style="width:68px"></div>';
	} else {
		img = '<div class="get selected" id="aid-'+id+'"><a class="del" href="javascript:;">删除</a> <img onclick="att_cancel(this,'+id+',\'upload\')" width="87" height="98" src="'+GV.DIMAUB+'statics/images/ext/'+ext+'.png" data-id="'+id+'" data-path="'+src+'" alt="上传完成" title="'+filename+'"><input type="hidden" id="fileId" value="'+id+'"/><input id="fileName"  type="text" class="J_file_desc" name="flashatt['+id+'][desc]" placeholder="请输入描述" value="'+filename+'"  style="width:68px"></div>';
	}
	//设置附件标识
