...
SWFUpload.Console.writeLine = function(g) {
	...
	try {
		...
		dialog({
			content : "Exception: " + h.name + " Message: " + h.message,
			width : 220,
			height : 40,
			ok : true,
			okValue : "确定",
...
(function(c) {
	...
	var a = {
		onDialogOpen : function() {
			var d = this.settings;
			this.queueData.errorMsg = "部分文件没有添加到上传队列:";
		...
		onDialogClose : function(d, f, g) {
			...
			if (c.inArray("onDialogClose", e.overrideEvents) < 0) {
				if (this.queueData.filesErrored > 0) {
					dialog({
						content : this.queueData.errorMsg,
						width : 220,
						height : 40,
						ok : true,
						okValue : "确定",
		...
		onSelect : function(h) {
			...
			if(c.browser.msie&&(c.browser.version == "8.0")){
				c(".proVessel").after('<div id="br_'+h.id+'"><br></div>');
			}
		...
		onUploadError : function(e, i, h) {
			...
			case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
				dialog({
					content : "The upload limit has been reached (" + h + ").",
					width : 220,
					height : 40,
					ok : true,
					okValue : "确定",
